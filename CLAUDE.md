...
- **Hook chips** (`src/pipeline/hooks.ts`) put rivalry framing first: the audit measured rivalry hooks at
  ...
  call, 24 Sept 2026: "no one cares about that" — and they change daily). "SWEPT"/"sweep" is a result word in
  `spoilsTheResult` (a best of 5 titled "PLAYOFFS | SWEPT vs TAS" told how it went). A typed hook always
  saves — any check on it (result words, length) comes back as a warning beside the field, never a refusal
  (the operator: "it should just be a suggestion"); so does the Title & description fold's save for a segment the generated title does not carry
...
- **Thumbnails are two big figures and two names** (`remotion/Thumbnail.tsx`, the thumbnail section of
  ...
  `match.seedType`; null or unknown leaves the slot out; the art is `SeedIconMC`, a small isometric
  diorama of the structure built from the 1.16.1 client's own textures in `remotion/assets/minecraft/`, each
  scene a plan of block characters per layer (`SCENES`, legend `BLOCKS`; preview with the `SeedIconSheet`
  composition) — a hand-drawn set was rejected on 24 Sept ("doesnt look like minecraft at all"), the
  one-item GUI slot found weak beside @mcsrmatches' scene tiles on 25 Sept) — no rating, seed, placement or other text. The four pose pairs
