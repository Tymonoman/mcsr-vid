...
- **Thumbnails are two big figures and two names** (`remotion/Thumbnail.tsx`, the thumbnail section of
  ...
  The shipwreck's main stays `SHIPWRECK-2d-dark` until the operator picks one of that day's eight 2d
  options (A is that one; B–H are `SHIPWRECK-2d-{full,full-outline,full-oak-outline,bow-on,beached,
  surface,broken-masts}.json`), then `icons.sh SHIPWRECK` after the map changes. A hand-drawn set was rejected on 24 Sept ("doesnt
