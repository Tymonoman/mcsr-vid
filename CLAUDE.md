...
| `bash scripts/browser-checks/run-all.sh <url>` | Drives the dashboard in a real browser the way the operator does (19 Playwright checks, self-configuring from `/api/matches`). Needs `npx playwright install chromium` once and a server with real data. |
