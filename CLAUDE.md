...
- The meta column's last state is the **subscribe card** (`postRollCta`, default on): three
  ...
  on screen changing. `renderOverlay({ only: ["splits"] })` redoes just the stills (seconds) for
  a match already on the shelf; the timer strip is the ~9 minutes and does not change.
...
- **Suggestions** are scored candidate matches. Each card carries a story line built server-side
  ...
  then every `suggestCacheTtlMin` (30) minutes for new matches only (an unforced scan returns the
  cache while fresh and walks the feed back to the last seen match once stale), and the
  `rescan` link forces everything; the nightly runs the same unforced scan before it picks. The
...
- **Upload** sends the tags from `match-<id>.tags.txt` (written by the pipeline, see
  ...
  one per player (`playerPlaylistTitle`) — the link a runner shares. A playlist created this
  way gets a description (`SEASON_PLAYLIST_DESCRIPTION`, `matchupPlaylistDescription`,
  `playerPlaylistDescription`); an existing one keeps its own.
  ...
  **A Studio upload is recognised without being ticked** (`src/channelUploads.ts`): the channel's
  ...
  uploads from before the pipeline, whose descriptions name no match. Back from Studio, the
  YouTube panel's "check the channel" link lists the channel at once (`GET
  /api/youtube/uploads?fresh=1`, three quota units) instead of waiting for the six-hour scan.
