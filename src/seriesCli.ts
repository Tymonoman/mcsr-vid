 ...
 *
 * A playoff series as one video, from any of its games' ids (src/series.ts): renders every game
 * that has no export yet, joins them into game 1's `series-<g1>.mp4`, writes game 1's title and
 * description for the series, and cuts the Short from the best game. `all` walks every seated
 ...
 *
 * Runs the pipeline in this process and `export:fast` / `short` as the dashboard does. The
...
const runners: SeriesRunners = {
  ...
  cutShort: (matchId) => npm("short", String(matchId), "--pick=0"),
