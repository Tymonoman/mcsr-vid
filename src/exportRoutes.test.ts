...
import { announcedTotalSec, percentOf, relocateRoot } from "./exportRoutes.js";
...
// --- Progress from two encoders' log lines. The nightly's export goes through the same job as the
// button's, and its ffmpeg never says "percentage:" — the bar sat at 0 for the whole encode.
assert.equal(percentOf("percentage: 42", 0), 42, "melt's own percentage, no total needed");
