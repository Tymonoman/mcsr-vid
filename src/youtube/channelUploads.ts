 ...
 * `Match page: https://magmamcsr.com/ranked/player/<nick>/matches/<id>` (src/pipeline/description.ts) and that description is
 ...
 *
 * Shaped like src/dashboard/rivalPosts.ts: an in-memory snapshot refreshed every six hours,
...
export interface ChannelVideo {
   ...
   * not propose again (src/youtube/publishSlot.ts). Optional: a video published on upload has no such
...
/** The Short of this match: same link in its description (src/shorts/shortHook.ts), three minutes or under. */
 ...
 * excludes its own match's booked slots (src/youtube/publishSlot.ts), and its own Short's day is not a
