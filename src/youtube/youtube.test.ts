...
// A playoff series is game 1's deliverable (src/playoffs/series.ts): the joined video beside the game's
...
const { totalReach } = await import("../dashboard/youtubeRoutes.js");
...
// still carrying "<HOOK>" is the one string that must never reach YouTube.
{
  ...
  const { handleYoutubeRoute } = await import("../dashboard/youtubeRoutes.js");
  const { HOOK_PLACEHOLDER } = await import("../pipeline/title.js");
...
{
  const { groupByHook } = await import("../dashboard/youtubeRoutes.js");
...
// naming the wrong video has to be turned away before any request is made.
{
  ...
  const { manifestPath } = await import("../thumbnails/thumbnailVariants.js");
  await writeFile(
