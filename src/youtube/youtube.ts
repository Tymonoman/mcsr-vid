 ...
 * src/youtube/channelUploads.ts, which walks three endpoints and has no reason to re-derive either;
...
export async function videoStats(videoIds: string[]): Promise<VideoStats[]> {
  ...
  return (body.items ?? []).map((v) => ({
    ...
    // Raw ISO 8601. Parsed by the caller (`parseIsoDuration`, src/youtube/channelUploads.ts) rather than
...
/**
 * YouTube's own cap on a video's tags: 500 characters across all of them, quotes included. Past
 * it the whole update is rejected, so the merge below drops candidates rather than the call.
 */
...
async function writeSnippet(
  ...
  await apiCall(
    ...
    "/videos?part=snippet",
    {
      method: "PUT",
      headers: { "content-type": "application/json" },
      // Every field the read returned goes back: this is a replace, not a merge.
      body: JSON.stringify({
        id: videoId,
        snippet: {
          title: merged.title,
