...
import { channelShortFor, channelUploadsSnapshot, channelVideoFor } from "../youtube/channelUploads.js";
import { config, matchDir } from "../config.js";
import { readManifest } from "../thumbnails/thumbnailVariants.js";
import { HOOK_PLACEHOLDER, metaPaths } from "../pipeline/title.js";
import { readUpload } from "../youtube/youtubeStore.js";
...
export type PublishChecklist = Record<ManualPublishKey, boolean> & {
  ...
  /** Both players' Twitch chat saved beside the media (src/api/twitchChat.ts) — the chat panel's input. */
  chatSaved: boolean;
};
 ...
 * link in its description (src/youtube/channelUploads.ts — how a Studio upload announces itself), or
...
export async function publishChecklist(
  ...
  // (src/pipeline/title.ts, `withHook`), so a match nobody has retyped a title for still counts as hooked.
