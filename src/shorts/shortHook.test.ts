...
import { buildTitle, HOOK_PLACEHOLDER } from "../pipeline/title.js";
...
// and the two halves of a match would otherwise drift apart ("#3 vs #17" vs "#3 vs #21").
{
  ...
  const user = (nickname: string, uuid: string) =>
    ({ nickname, uuid, eloRate: 1900, eloRank: 40 }) as unknown as import("../api/types.js").UserDetails;
  const match = {
    forfeited: false,
    changes: [],
    ...
    timelines: [],
  } as unknown as import("../api/types.js").MatchInfo;
  const moment = {
    startMs: 0,
    endMs: 22_000,
    score: 1,
