...
const plan = (id, extra = {}) => ({
  ...
  shortHook: null,
  titleHook: null,
  suggestions: { short: ["3.8 SECONDS APART AT THE EYES", "A SUB-8 TO WIN IT"], title: ["A sub-8 to win it"] },
...
(async () => {
  ...
  const wire = async (page) => {
    ...
    await page.route("**/api/matches", async (r) => {
      ...
      for (const m of d.matches)
        if (plans[m.matchId]) Object.assign(m, { shortState: plans[m.matchId].state, shortDetail: "Short 6:21–7:02" });
      return r.fulfill({ json: d });
    });
    await page.route("**/api/nightly", async (r) => {
      const d = await (await r.fetch()).json();
      const ids = (s) => Object.values(plans).filter((p) => p.state === s).map((p) => p.matchId);
      return r.fulfill({
        json: {
          ...d,
  ...
  try {
    ...
    check("the strip counts the matches waiting for a hook", /2 waiting for a hook/.test(strip), strip.slice(0, 200));
    ...
    check("the picker's failure sits in the Pick step", /not signed in/.test(pickFail), pickFail.slice(0, 80));
    check("the heuristic's pick is marked as such", /heuristic stands in/.test(await page.textContent('#now .step[data-step="pick"] .sum')));
    ...
    check("the button says what it does", (await page.textContent("#save")).trim() === "Save hooks and schedule");

    await page.click("#save");
    await page.waitForFunction(() => document.querySelector("#savedmsg")?.textContent === "saved", null, { timeout: 10000 });
    ...
    await page.waitForFunction(() => /Next waiting/.test(document.querySelector("#save")?.textContent ?? ""), null, {
      timeout: 10000,
    });
    await page.click("#save");
    ...
    plans[a] = plan(a, {
      ...
      state: "failed",
      errors: [{ step: "upload-video", at: new Date().toISOString(), message: "YouTube said 403 quotaExceeded" }],
    });
    puts.length = 0;
    await page.evaluate((id) => select(id, { open: true }), a);
    await page.waitForSelector('#now .step[data-step="videoup"] .inlinefail', { timeout: 30000 });
    check("a failed upload reports under Video up", /quotaExceeded/.test(await page.textContent('#now .step[data-step="videoup"]')));
    await page.click('#now .step[data-step="videoup"] [data-act="retry"]');
    await page.waitForTimeout(500);
    check("Retry re-saves the saved hooks", puts[0]?.body.titleHook === "A sub-8 to win it", JSON.stringify(puts[0]?.body));
    ...
    plans[a] = plan(a, {
      ...
      shortHook: "A SUB-8 TO WIN IT",
      state: "failed",
      detail: "uploads are off (youtubeUploadEnabled / nightlyUpload) — the Short is rendered; Publish is the way",
    });
    await page.evaluate((id) => select(id, { open: true }), a);
    ...
    check(
      ...
      await page.evaluate(
        () =>
          hookOfTitle("PLAYOFFS | SWEPT vs TAS | A vs B | Playoffs | Round of 16", "A vs B | Playoffs | Round of 16") ===
    ...
    // Every other row waiting for a hook: the two above, and whatever the server's own list adds.
    const others = await phone.evaluate(
      () => matches.filter((m) => !m.hidden && m.shortState === "waiting-for-hook" && m.matchId !== selected).length,
    );
    const more = await phone.$eval("#nextwaiting", (e) => (e.checkVisibility() ? e.textContent : ""));
    check("the back bar offers the next match waiting", others >= 1 && more === `${others} more waiting ›`, more);
    const toggles = await phone.$$eval("#now .steptoggle", (n) => Math.min(...n.map((e) => e.getBoundingClientRect().height)));
