...
(async () => {
  ...
  try {
    ...
    check("every writable setting has an input", (await page.locator("#settings [data-key]").count()) === before.fields.length,
      `${await page.locator("#settings [data-key]").count()} of ${before.fields.length}`);
    for (const key of ["youtubeUploadEnabled", "nightlyUpload"]) {
      check(`${key} is not writable from the browser`, (await page.locator(`#settings input[data-key="${key}"]`).count()) === 0);
    }
    check("but it is still reported", (await page.locator("#settings .setrow.off").count()) >= 2);
    check("the nightly's arming is stated", /Nightly (armed for|is)/.test(await page.locator("#settings .scanline").first().innerText()));
    ...
    check("a real save reports what moved", /saved suggestChaosSlots/.test(await page.locator("#setmsg").innerText()));
    check("and the server agrees", String((await settings()).fields.find((f) => f.key === "suggestChaosSlots").value) === next);
    ...
    if (before) {
      ...
      for (const f of before.fields) {
        restore[f.key] = f.kind === "boolean" ? !!f.value : f.kind === "words" ? (f.value ?? []).join(" ") : f.value === null ? "" : String(f.value);
      }
