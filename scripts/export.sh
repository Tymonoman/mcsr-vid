...
# The desktop preset is h264_nvenc on an RTX 2060; there is no NVIDIA GPU here, so
# VAAPI on the Intel HD 520 is the like-for-like swap. Falls back to CPU when
# /dev/dri isn't passed into the container.
if [ -e /dev/dri/renderD128 ] && vainfo 2>/dev/null | grep -qi h264; then
  CODEC=(vcodec=h264_vaapi)
else
...
XDG_RUNTIME_DIR=/tmp xvfb-run -a "$MELT" "$PROJECT" ${RANGE:+out=$RANGE} \
