...
cd "$wt"
HOME=/app/.tools/agy-dev /app/.tools/bin/agy -p "You are working in the git worktree $wt (branch agy/$name) of the mcsr-vid repo.
...
wc, npx tsx <file>, npx prettier, npx tsc --noEmit, npm run typecheck, npm run test:unit. No loops, pipes,
&&, ; or redirects: anything else is refused and ends your turn with nothing done. Prefer your own
...
TASK:
$(cat "$prompt")" \
  --model "$model" --mode accept-edits --output-format json --print-timeout 1800s >"$out" || true
echo "agy done: $out"
